...
function NewWebsiteContent() {
  ...
  const handleStep1Submit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!cleanHost || isSubmitting) return;

    setIsSubmitting(true);
    setErrorMessage(null);

    try {
      const res = await websitesApi.create({
        domain: cleanHost,
        timezone,
      });
      ...
      if (res.success && res.website) {
        ...
        setErrorMessage(res.error || "Failed to create website. Please check your domain.");
      }
    } catch (err: any) {
  ...
  defer
  data-website-id="${siteId || "site_live_analytics"}"
  ...
  return (
    <div className="relative min-h-[calc(100vh-140px)] flex flex-col justify-center items-center py-6 px-4">
      ...
      <div className="relative z-10 w-full max-w-xl space-y-6">
          ...
          <div className="flex items-center gap-1.5 text-xs font-mono">
            <span
              className={`px-2.5 py-0.5 rounded-full ${
                step === 1 ? "bg-[#800E13] text-white font-bold" : "bg-[#262626] text-zinc-400"
              }`}
            >
              1. Website
            </span>
            ...
            <span
              className={`px-2.5 py-0.5 rounded-full ${
                step === 2 ? "bg-[#800E13] text-white font-bold" : "bg-[#262626] text-zinc-400"
              }`}
            >
              2. Tracking Code
            </span>
        ...
        {/* Error Alert if any */}
        {errorMessage && (
          <div className="rounded-xl bg-rose-500/10 border border-rose-500/30 p-3.5 flex items-start gap-3 text-xs text-rose-300">
          ...
          <div className="rounded-2xl bg-[#262626] border border-white/[0.08] p-6 shadow-2xl space-y-5">
            <div>
              ...
              <p className="text-xs text-zinc-400 mt-0.5">
                We'll configure real-time cookieless analytics and MRR attribution for your domain.
              </p>
            </div>
            ...
            <form onSubmit={handleStep1Submit} className="space-y-4">
              ...
              <div className="pt-2">
                <Button
                  type="submit"
                  disabled={!cleanHost || isSubmitting}
                  className="w-full h-11 bg-[#800E13] hover:bg-[#9e1218] text-white font-medium text-sm rounded-xl transition-all border border-[#800E13] flex items-center justify-center gap-2 cursor-pointer shadow-md disabled:opacity-50"
                >
                  {isSubmitting ? (
                    <>
        ...
        {step === 2 && (
          <div className="rounded-2xl bg-[#262626] border border-white/[0.08] p-6 shadow-2xl space-y-5">
            <div>
              ...
              <p className="text-xs text-zinc-400 mt-0.5">
                Drop this lightweight script into your HTML head or use our typed NPM package.
              </p>
            </div>
            ...
            <div className="w-full rounded-xl bg-[#141414] border border-white/[0.1] overflow-hidden shadow-2xl">
              ...
              <div className="flex items-center justify-between px-3 sm:px-4 py-2 border-b border-white/[0.06] bg-[#1f1f1f] gap-2">
                ...
                {/* Tab Selector: HTML Script & NPM SDK */}
                <div className="bg-[#141414] p-0.5 border border-white/[0.06] rounded-lg h-7.5 flex items-center gap-1">
                  <button
                    ...
                    className={`text-xs px-3 py-1 font-medium rounded-md transition-all flex items-center gap-1.5 cursor-pointer ${
                  ...
                  <div className="p-4 font-mono text-xs leading-relaxed overflow-x-auto flex items-start gap-4 bg-[#141414]">
                    ...
                    <div className="flex-1 text-zinc-200 min-w-0 whitespace-pre">
                      ...
                      <div className="pl-4">
                        ...
                        <span className="text-zinc-500">=</span>
                        <span className="text-emerald-400">&quot;{siteId}&quot;</span>
                      </div>
                      <div className="pl-4">
                ...
                <div>
                  ...
                  <div className="p-4 font-mono text-xs leading-relaxed overflow-x-auto flex items-start gap-4 bg-[#141414]">
                    ...
                    <div className="flex-1 text-zinc-200 min-w-0 whitespace-pre">
                      ...
                      <div>
                        <span className="text-rose-400 font-semibold">initAnalytics</span>(&#123;{" "}
                        websiteId: <span className="text-emerald-400">&quot;{siteId}&quot;</span> &#125;);
                      </div>
                    </div>
                  </div>
                </div>
